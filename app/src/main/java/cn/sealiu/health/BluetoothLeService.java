...
public class BluetoothLeService extends Service {
    ...
    public static final String PRESSURE_MEASUREMENT = "0000ffe1-0000-1000-8000-00805f9b34fb";
    ...
    private void broadcastUpdate(String action, int rssi) {
        ...
        localIntent.putExtra(EXTRA_DATA, String.valueOf(rssi));
    ...
    private void broadcastUpdate(final String action,
        ...
        if (characteristic.getUuid().toString().equals(PRESSURE_MEASUREMENT)) {
            final Intent intent = new Intent(action);
            final byte[] data = characteristic.getValue();

            // 每次传递过来的数据长度不一定固定（一般为：14，20，17）
            if (D) Log.d(TAG, "测试数据：data.length = " + data.length);
            final StringBuilder stringBuilder = new StringBuilder(data.length);
            for (byte aData : data)
                stringBuilder.append(String.format("%02X", aData));

            intent.putExtra(EXTRA_DATA, stringBuilder.toString());
            Log.e(TAG, "broadcastUpdate for read data:" + stringBuilder.toString());
